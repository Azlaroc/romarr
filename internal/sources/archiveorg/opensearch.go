...
// WithOpenSearch turns the widening step on or off. Off makes mapped
// collections the only source again — the pre-2.0 behaviour, kept reachable
// as a registry edit rather than a code change.
func WithOpenSearch(on bool) Option {
	return func(d *Driver) { d.openEnabled = on }
}
