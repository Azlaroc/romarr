...
func TestScanLibraryDirs(t *testing.T) {
	...
	// ROM tree content is the library scanner's, never the boot scan's.
	...
	if total := jobs.LibraryTotal(); total != len(wantSourceIDs) {
		t.Errorf("library total = %d, want %d (vault only — the ROM tree is not the boot scan's)", total, len(wantSourceIDs))
	}
